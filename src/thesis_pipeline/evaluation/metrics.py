...
def calculate_metrics(
    ...
    completed = [record for record in records if record.remediation_completed <= horizon_end]
    backlog = [record for record in records if record.remediation_completed > horizon_end]
    ...
    remediation_times = [
        _hours(record.alert_created, record.remediation_completed) for record in completed
    ]

    evaluated_for_sla = [
        record
        for record in records
        if record.remediation_completed <= horizon_end or record.sla_deadline <= horizon_end
    ]
    breached = [
        record for record in evaluated_for_sla if record.remediation_completed > record.sla_deadline
    ]
    ...
    actionable_total = sum(record.finding.actionable for record in records)
    actionable_completed = sum(record.finding.actionable for record in completed)
    ...
    remediator_busy_hours = sum(
        ...
        for record in records
        if record.remediation_started < horizon_end
    )
    ...
    exposure_hours = sum(
        ...
        * _hours(record.alert_created, min(record.remediation_completed, horizon_end))
        for record in records
    )
    ...
    metrics: dict[str, float | int | str] = {
        ...
        "completed_within_horizon": len(completed),
        "backlog_at_horizon": len(backlog),
        "throughput_cases_per_hour": len(completed) / config.horizon_hours,
